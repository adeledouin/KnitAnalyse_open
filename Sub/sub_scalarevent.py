...
from Datas.classSignal import VariationsScalar
from Datas.classEvent import ForceEvent
from Sub.sub_plot_variations_event import plot_pdf_event, plot_variations_df_tt, plot_pdf_event_comparaison, plot_variations_df_img

################### Main code ##################################

def scalarevent(config, config_plot, signaltype, NN_data, histo, plot, display_figure, display_figure_reg,
                signal_flu, signal_img, Sm):

    display_figure_variations = display_figure

# %% ################### Partie 0 : Detect events ##################################
    print('#-------------detect events--------------#')

    signalevent = ForceEvent(config, signal_flu.f, signal_flu.ext, signal_flu.t, signaltype, NN_data, Sm=Sm,
                             display_figure_debug=False)
    ...
    variations_df_tt = VariationsScalar(config, pourcentage=5, f=signalevent.df_tt, ext=signal_flu.ext, t=signal_flu.t,
                                        index=signalevent.index_df_tt, number=signalevent.number_df_tt,
                                        directsignal=False,
    ...
    exposants, seuils, save_seuils, nb_seuils, which_seuil, nbclasses = config.config_scalarevent(Sm if signaltype == 'flu' else True)
    ...
    if display_figure:
        ...
        if display_figure_variations:
            plot_variations_df_tt(plot, signalevent, variations_df_tt, which_seuil, seuils, exposants,
                                  display_figure_reg)
    ...
    if config.img:
        ...
        variations_nb_df_tt_btw = VariationsScalar(config, pourcentage=5, f=nb_df_tt_btw, ext=signal_flu.ext,
                                                   t=signal_flu.t,
                                                   index=signal_img.index_picture, number=signal_img.numbertot_picture,
                                                   directsignal=False,
                                                   signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                   fname='nb_df_tt_btw', stats=True)

        variations_nb_df_seuil_btw = VariationsScalar(config, pourcentage=5, f=nb_df_seuil_btw, ext=signal_flu.ext,
                                                      t=signal_flu.t,
                                                      index=signal_img.index_picture,
                                                      number=signal_img.numbertot_picture,
                                                      directsignal=False,
                                                      signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                      fname='nb_df_seuil_btw', stats=True)

        variations_sum_df_tt_btw = VariationsScalar(config, pourcentage=5, f=sum_df_tt_btw, ext=signal_flu.ext,
                                                    t=signal_flu.t,
                                                    index=index_df_tt_btw, number=number_df_tt_btw, directsignal=False,
                                                    signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                    fname='sum_df_tt_btw', stats=True)
        ...
        variations_sum_df_seuil_btw = VariationsScalar(config, pourcentage=5, f=sum_df_seuil_btw, ext=signal_flu.ext,
                                                       ...
                                                       directsignal=False,
                                                       signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                       fname='sum_df_seuil_btw', stats=True)

        variations_max_df_tt_btw = VariationsScalar(config, pourcentage=5, f=max_df_tt_btw, ext=signal_flu.ext,
                                                    t=signal_flu.t,
                                                    index=index_df_tt_btw, number=number_df_tt_btw, directsignal=False,
                                                    signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                    fname='max_df_tt_btw', stats=True)
        ...
        variations_max_df_seuil_btw = VariationsScalar(config, pourcentage=5, f=max_df_seuil_btw, ext=signal_flu.ext,
                                                       ...
                                                       directsignal=False,
                                                       signaltype=signaltype, NN_data=NN_data, ftype='img',
                                                       fname='max_df_seuil_btw', stats=True)
        ...
        if display_figure:
            ...
            plot_pdf_event_comparaison(plot, nb_df_tt_btw, nb_df_seuil_btw, variations_nb_df_tt_btw,
                                       variations_nb_df_seuil_btw,
        ...
        # %% ##### sum_df
        if display_figure:
            plot_pdf_event_comparaison(plot, sum_df_tt_btw, sum_df_seuil_btw, variations_sum_df_tt_btw,
                                       variations_sum_df_seuil_btw,
            ...
            plot_variations_df_img(plot, signal_img, signalevent, variations_sum_df_tt_btw, variations_sum_df_seuil_btw,
                                   '\Sigma \delta f_{btwpict}', 'sum_df_btwpict', seuil, exposant,
                                   save=signal_img.to_save_fig)

        # %% ##### max_df
        if display_figure:
            plot_pdf_event_comparaison(plot, max_df_tt_btw, max_df_seuil_btw, variations_max_df_tt_btw,
                                       variations_max_df_seuil_btw,
            ...
            plot_variations_df_img(plot, signal_img, signalevent, variations_max_df_tt_btw, variations_max_df_seuil_btw,
                                   'Max( \delta f_{btwpict})', 'max_df_btwpict', seuil, exposant,
                                   save=signal_img.to_save_fig)
