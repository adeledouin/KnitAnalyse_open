...
def NNrescaling(config, plot, signaltype, NN_data, signal_flu, signal_img, display_figure):
...
# %% ################### Partie 1 : Rsc flu ##################################
    if not config.mix_set:
        ...
        for i in range(np.size(NN_data)):
            ...
            nb_index_picture_rsc = variations_flu.rsc_par_fenetre_tps(signal_flu[i].f, signal_flu[i].ext,
                                                                      signal_flu[i].t,
                                                                      signal_img[i].index_picture,
                                                                      signal_img[i].number_picture)

            signal_flu[i].save_data('flu_rsc', NN, f_rsc, t_rsc, ext_rsc, f_rsc_size, index_picture_rsc,
                                    number_picture_rsc,
        ...
        for i in range(config.nb_set):
            variations_mix[i] = VariationsScalar(config, pourcentage=5,
                                                 f=signal_flu[0].f[signal_flu[0].NN_sub_cycles[i], :],
                                                 ext=signal_flu[0].ext[signal_flu[0].NN_sub_cycles[i], :],
                                                 t=signal_flu[0].t[signal_flu[0].NN_sub_cycles[i], :],
                                                 index=None, number=None, directsignal=True,
                                                 signaltype=signaltype, NN_data=NN_data[0], ftype='force',
                                                 fname=signaltype,
                                                 rsc=True)
        ...
        for j in range(np.size(NN_data)):
            ...
            if display_figure:
                plot.plot_x_y_multiarray(signal_flu_rsc[j].nbcycle, signal_flu_rsc[j].ext, signal_flu_rsc[j].f,
                                         'L_{w} (mm)', '\delta f', 'Lw', signal_flu_rsc[j].fname,
                                         save=signal_flu_rsc[j].to_save_fig)
            ...
            if display_figure_variations:
                variations_flu_rsc = VariationsScalar(config, pourcentage=5, f=signal_flu_rsc[j].f,
                                                      ...
                                                      index=None, number=None, directsignal=True,
                                                      signaltype=signaltype, NN_data=NN, ftype='force',
                                                      fname=signaltype)
...
# %% ################### Partie 2 : ReshapeImg ##################################
    if config.img:
        for i in range(np.size(NN_data)):
            ...
            signal_img[i].reshape_all_fields(names_tosave, signal_flu_rsc[i], prev_field, sub_NN=True)
